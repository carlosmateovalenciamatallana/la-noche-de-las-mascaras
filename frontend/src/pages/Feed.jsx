...
export default function Feed() {
  ...
  useEffect(() => {
    const fetchFeed = async () => {
      try {
        const response = await axios.get('http://192.168.1.5:3001/api/feed');
  ...
  const handleLike = async (targetId) => {
    try {
      ...
      await axios.post('http://192.168.1.5:3001/api/likes', { profileId: targetId, userId: deviceId });
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const deviceId = localStorage.getItem('deviceId');
      await axios.post('http://192.168.1.5:3001/api/comments', { 
  ...
  const handlePinSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://192.168.1.5:3001/api/validate-pin', { pin: pinInput });
